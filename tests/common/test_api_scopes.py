...
class TestApiScopes(unittest.TestCase):
    ...
    def test_scope_mapping_consistency(self):
        ...
        # Validate that genuinely read-only resources only ever use 'read'.
        # NOTE: Alerts, Hosts, and Incidents previously lived here but now expose
        # write operations (alert status updates, host containment/tagging,
        # incident actions) once mutations are enabled, so they are no longer
        # read-only. This list contains only resources with no write operation
        # anywhere in the modules.
        read_only_resources = [
            "Vulnerabilities", "Sensor Usage", "Scheduled Reports",
            "Sensor Download", "Falcon Data Replicator", "Threatgraph",
            "Tailored Intelligence (Typosquatting)",
        ]
