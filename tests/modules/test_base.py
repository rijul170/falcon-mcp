...
from falcon_mcp.modules.base import (
    READ_ONLY_ANNOTATIONS,
    BaseModule,
    is_allow_destructive,
    is_readonly_mode,
)
from tests.modules.utils.test_modules import TestModules

# A non-destructive mutating tool annotation (write, but not destructive) for gate tests.
...
# A destructive mutating tool annotation for the second-tier gate tests.
...
class TestBaseModule(TestModules):
    ...
    @mock.patch.dict("os.environ", {"FALCON_MCP_READONLY": "true"})
    def test_readonly_mode_suppresses_mutating_tool(self):
        """Read-only mode must skip registration of mutating tools entirely."""
        self.module._add_tool(
            server=self.mock_server,
            method=lambda: None,
            name="mutating_tool",
        ...
        # Mutating tool must NOT be registered and must NOT be tracked.
        self.mock_server.add_tool.assert_not_called()
        self.assertNotIn("falcon_mutating_tool", self.module.tools)

    @mock.patch.dict("os.environ", {"FALCON_MCP_READONLY": "true"})
    def test_readonly_mode_still_registers_read_tool(self):
        """Read-only mode must still register read-only (default-annotation) tools."""
        self.module._add_tool(
            server=self.mock_server,
            method=lambda: None,
            name="read_tool",
        ...
        self.mock_server.add_tool.assert_called_once()
        self.assertIn("falcon_read_tool", self.module.tools)

    @mock.patch.dict("os.environ", {"FALCON_MCP_READONLY": "false"})
    def test_readonly_off_registers_mutating_tool(self):
        """When read-only mode is off, mutating tools register normally."""
        self.module._add_tool(
            server=self.mock_server,
            method=lambda: None,
            name="mutating_tool",
        ...
        self.mock_server.add_tool.assert_called_once()
        self.assertIn("falcon_mutating_tool", self.module.tools)

    def test_is_readonly_mode_truthy_values(self):
        """is_readonly_mode parses truthy values case-insensitively."""
        for val in ("true", "TRUE", "1", "yes", "Yes"):
            with mock.patch.dict("os.environ", {"FALCON_MCP_READONLY": val}):
                self.assertTrue(is_readonly_mode(), f"{val!r} should be truthy")

        for val in ("false", "0", "no", "", "off"):
            with mock.patch.dict("os.environ", {"FALCON_MCP_READONLY": val}):
                self.assertFalse(is_readonly_mode(), f"{val!r} should be falsy")

    @mock.patch.dict(
        "os.environ",
        {"FALCON_MCP_READONLY": "false", "FALCON_MCP_ALLOW_DESTRUCTIVE": ""},
    )
    def test_destructive_tool_suppressed_by_default_when_writes_enabled(self):
        """With writes enabled but ALLOW_DESTRUCTIVE unset, destructive tools are suppressed."""
        self.module._add_tool(
            server=self.mock_server,
            method=lambda: None,
        ...
        self.mock_server.add_tool.assert_not_called()
        self.assertNotIn("falcon_destructive_tool", self.module.tools)

    @mock.patch.dict(
        "os.environ",
        {"FALCON_MCP_READONLY": "false", "FALCON_MCP_ALLOW_DESTRUCTIVE": "true"},
    )
    def test_destructive_tool_registered_when_allowed(self):
        """ALLOW_DESTRUCTIVE=true registers destructive tools (full production tier)."""
        self.module._add_tool(
            server=self.mock_server,
            method=lambda: None,
            name="destructive_tool",
        ...
        self.mock_server.add_tool.assert_called_once()
        self.assertIn("falcon_destructive_tool", self.module.tools)

    @mock.patch.dict(
        "os.environ",
        {"FALCON_MCP_READONLY": "false", "FALCON_MCP_ALLOW_DESTRUCTIVE": "true"},
    )
    def test_readonly_still_overrides_allow_destructive_is_independent(self):
        """Non-destructive writes register with writes enabled regardless of destructive flag."""
        self.module._add_tool(
            server=self.mock_server,
            method=lambda: None,
            name="write_tool",
        ...
        self.mock_server.add_tool.assert_called_once()
        self.assertIn("falcon_write_tool", self.module.tools)

    @mock.patch.dict(
        "os.environ",
        {"FALCON_MCP_READONLY": "true", "FALCON_MCP_ALLOW_DESTRUCTIVE": "true"},
    )
    def test_readonly_takes_precedence_over_allow_destructive(self):
        """Read-only mode suppresses destructive tools even if ALLOW_DESTRUCTIVE is set."""
        self.module._add_tool(
            server=self.mock_server,
            method=lambda: None,
            name="destructive_tool",
    ...
    @mock.patch.dict(
        "os.environ",
        {"FALCON_MCP_READONLY": "false", "FALCON_MCP_ALLOW_DESTRUCTIVE": "falcon_delete_thing, delete_other"},
    )
    def test_destructive_allowlist_permits_only_named_tools(self):
        """A comma-list value arms only the named destructive tools (prefix optional)."""
        # Named with explicit falcon_ prefix -> registered.
        self.module._add_tool(
            server=self.mock_server, method=lambda: None, name="delete_thing",
            annotations=DESTRUCTIVE_ANNOTATIONS,
        )
        # Named without prefix -> registered (prefix is added during parsing).
        self.module._add_tool(
            server=self.mock_server, method=lambda: None, name="delete_other",
            annotations=DESTRUCTIVE_ANNOTATIONS,
        )
        # Not in the allowlist -> suppressed.
        self.module._add_tool(
            server=self.mock_server, method=lambda: None, name="delete_unlisted",
            annotations=DESTRUCTIVE_ANNOTATIONS,
        )
        self.assertIn("falcon_delete_thing", self.module.tools)
        self.assertIn("falcon_delete_other", self.module.tools)
        self.assertNotIn("falcon_delete_unlisted", self.module.tools)

    def test_is_allow_destructive_truthy_values(self):
        """is_allow_destructive parses truthy values case-insensitively."""
        for val in ("true", "TRUE", "1", "yes", "Yes"):
            with mock.patch.dict("os.environ", {"FALCON_MCP_ALLOW_DESTRUCTIVE": val}):
                self.assertTrue(is_allow_destructive(), f"{val!r} should be truthy")

        for val in ("false", "0", "no", "", "off"):
            with mock.patch.dict("os.environ", {"FALCON_MCP_ALLOW_DESTRUCTIVE": val}):
                self.assertFalse(is_allow_destructive(), f"{val!r} should be falsy")
