...
class TestNGSIEMModule(TestModules):
    ...
    def test_register_tools(self):
        ...
        expected_tools = [
            "falcon_create_ngsiem_connection",
            "falcon_delete_ngsiem_connection",
            "falcon_get_ngsiem_data_connection_status",
            "falcon_get_ngsiem_ingest_token",
            "falcon_list_ngsiem_dashboards",
            "falcon_list_ngsiem_data_connections",
            "falcon_list_ngsiem_data_connectors",
            "falcon_list_ngsiem_lookup_files",
            "falcon_list_ngsiem_parsers",
            "falcon_list_ngsiem_saved_queries",
            "falcon_regenerate_ngsiem_ingest_token",
            "falcon_search_ngsiem",
            "falcon_set_ngsiem_connection_state",
        ]
        self.assert_tools_registered(expected_tools)

    def test_list_ngsiem_lookup_files(self):
        self.mock_client.command.return_value = {"status_code": 200, "body": {"resources": []}}
        self.module.list_ngsiem_lookup_files(repository=None, limit=100, offset=0)
        call = self.mock_client.command.call_args_list[0]
        self.assertEqual(call[0][0], "ListLookupFiles")

    def test_list_ngsiem_data_connections(self):
        self.mock_client.command.return_value = {"status_code": 200, "body": {"resources": []}}
        self.module.list_ngsiem_data_connections(filter=None, limit=100, offset=0, sort=None)
        call = self.mock_client.command.call_args_list[0]
        self.assertEqual(call[0][0], "ExternalListDataConnections")

    def test_get_data_connection_status(self):
        self.mock_client.command.return_value = {"status_code": 200, "body": {"resources": []}}
        self.module.get_ngsiem_data_connection_status(ids=["dc1"])
        call = self.mock_client.command.call_args_list[0]
        self.assertEqual(call[0][0], "ExternalGetDataConnectionStatus")
        self.assertEqual(call[1]["parameters"]["ids"], ["dc1"])
