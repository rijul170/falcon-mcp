...
class TestCloudModule(TestModules):
    ...
    def test_count_kubernetes_containers(self):
        """Test count for kubernetes containers returns the integer count."""
        mock_response = {"status_code": 200, "body": {"resources": [500]}}
        self.mock_client.command_for.return_value = mock_response

        result = self.module.count_kubernetes_containers(filter="cloud_region:'us-1'")

        self.assertEqual(self.mock_client.command_for.call_count, 1)

        first_call = self.mock_client.command_for.call_args_list[0]
        self.assertEqual(first_call[0][0], "ReadContainerCount")
        self.assertEqual(first_call[1]["parameters"]["filter"], "cloud_region:'us-1'")
        # The count is extracted from the resources list and returned as an int.
        self.assertEqual(result, 500)

    def test_count_kubernetes_containers_errors(self):
        """Test count for kubernetes containers returns an error dict on API error."""
        mock_response = {
            "status_code": 500,
