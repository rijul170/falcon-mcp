...
from falcon_mcp.modules.base import READ_ONLY_ANNOTATIONS, is_readonly_mode
...
class FalconMCPServer:
    ...
    def __init__(
        ...
        # Surface read-only mode at startup. The actual gate lives in
        # BaseModule._add_tool (driven by the FALCON_MCP_READONLY env var);
        # the --read-only CLI flag sets that env var before this point.
        if is_readonly_mode():
            logger.info(
                "=== READ-ONLY MODE ACTIVE === Mutating tools will NOT be "
                "registered; only read-only tools are exposed."
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--read-only",
        action="store_true",
        default=is_readonly_mode(),
        help="Enable read-only mode: mutating tools are not registered, only "
        "read-only tools are exposed (env: FALCON_MCP_READONLY=true/1/yes)",
    )
...
def main() -> None:
    ...
    # The read-only gate in BaseModule._add_tool reads FALCON_MCP_READONLY from
    # the environment. Ensure the CLI flag (or its env default) is reflected there
    # so module registration sees a consistent value.
    if args.read_only:
        os.environ["FALCON_MCP_READONLY"] = "true"
