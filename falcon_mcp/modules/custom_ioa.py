...
class CustomIOAModule(BaseModule):
    ...
    def register_tools(self, server: FastMCP) -> None:
        ...
        self._add_tool(
            server=server,
            method=self.assign_ioa_rule_group_to_prevention_policy,
            name="assign_ioa_rule_group_to_prevention_policy",
            annotations=ToolAnnotations(
                readOnlyHint=False,
                destructiveHint=False,
                idempotentHint=True,
    ...
    def assign_ioa_rule_group_to_prevention_policy(
        self,
        policy_id: str = Field(description="Prevention policy ID to attach IOA rule groups to."),
        rule_group_ids: list[str] = Field(description="Custom IOA rule group IDs to attach."),
        member_cid: str | None = Field(
            default=None,
            description="Optional child CID for MSSP child tenant scoping.",
        ),
    ) -> list[dict[str, Any]]:
        """Attach Custom IOA rule groups to a Prevention Policy.

        Custom IOA rule groups are assigned to prevention policies, not directly to
        host groups. The assignment chain is:
          Host Group → Prevention Policy → Custom IOA Rule Group

        Use `falcon_search_prevention_policies` to find the target policy ID and
        `falcon_search_ioa_rule_groups` to find the rule group IDs.
        """
        if not rule_group_ids:
            return [_format_error_response("`rule_group_ids` is required.", operation="performPreventionPoliciesAction")]
        results: list[dict[str, Any]] = []
        for rg_id in rule_group_ids:
            result = self._base_query_api_call(
                operation="performPreventionPoliciesAction",
                query_params={"action_name": "add-rule-group"},
                body_params={
                    "ids": [policy_id],
                    "action_parameters": [{"name": "rule_group_id", "value": rg_id}],
                },
                error_message=f"Failed to assign IOA rule group {rg_id} to prevention policy",
