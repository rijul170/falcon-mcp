...
class CloudModule(BaseModule):
    ...
    def count_kubernetes_containers(
        ...
        response = self.client.command_for(operation, member_cid=member_cid, parameters=params)

        # Handle the response. On error, handle_api_response returns an error dict;
        # return it cleanly (matches the declared `int | dict` return type).
        ...
        # ReadContainerCount returns the count inside the resources list (e.g. [500]).
        # Extract the integer count; default to 0 when no count is present.
    ...
    def search_images_vulnerabilities(
        ...
        # Handle the response. On a 403/empty/error this returns an error dict,
        # which now matches the declared `list | dict` return type.
