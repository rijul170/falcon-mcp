...
class DetectionsModule(BaseModule):
    ...
    def register_tools(self, server: FastMCP) -> None:
        ...
        # NOTE: Alert mutation (the v3 PatchEntitiesAlertsV3 action) is owned by the
        # unified `alerts` module via `falcon_update_alerts`. A duplicate copy used to
        # live here, which collided on the same tool name and additionally sent
        # incorrect action-parameter names (`assignee_uuid`/`comment` instead of the
        # API's `assign_to_uuid`/`append_comment`). Use `falcon_update_alerts`.
