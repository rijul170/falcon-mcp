...
class IdpModule(BaseModule):
    ...
    def register_tools(self, server: FastMCP) -> None:
        ...
        """
        self._add_tool(server=server, method=self.investigate_entity, name="idp_investigate_entity")
        self._add_tool(server=server, method=self.list_identity_protection_policy_rules, name="list_identity_protection_policy_rules")
        self._add_tool(server=server, method=self.get_identity_protection_policy_rule_details, name="get_identity_protection_policy_rule_details")
        self._add_tool(
            server=server, method=self.create_identity_protection_policy_rule, name="create_identity_protection_policy_rule",
            annotations=ToolAnnotations(readOnlyHint=False, destructiveHint=False, idempotentHint=False, openWorldHint=True),
        )
        self._add_tool(
            server=server, method=self.delete_identity_protection_policy_rules, name="delete_identity_protection_policy_rules",
    ...
    def _resolve_entities(self, identifiers: dict[str, Any]) -> list[str] | dict[str, Any]:
        ...
        # If we have both USER and ENDPOINT criteria, return an explicit error rather than
        # silently dropping IP addresses. The GraphQL entity model separates USER and ENDPOINT
        # queries, so mixing them in a single call is unsupported.
        if has_user_criteria and has_endpoint_criteria:
            return {
                "error": (
                    "Cannot combine email_addresses (USER entities) and ip_addresses (ENDPOINT entities) in a single query. "
                    "Run separate calls: one with email_addresses, one with ip_addresses."
                )
            }
    ...
    # ==========================================
    # Identity Protection Policy Rule Tools
    # ==========================================

    def list_identity_protection_policy_rules(
        self,
        ...
        """List all Identity Protection policy rules in the environment.

        IdP policy rules define behavioral triggers (e.g., remote authentication,
        lateral movement) that generate Identity-based incidents. Rules are NOT
        bound to Falcon host groups — they apply to Active Directory entities.

        Use this to audit existing rules or extract them for replication across
        child tenants.
        """
        ids_result = self._base_search_api_call(
            operation="get_policy_rules_query",
            search_params={"limit": 5000},
            error_message="Failed to query identity protection policy rule IDs",
            member_cid=member_cid,
        )
        if self._is_error(ids_result):
            return [ids_result]
        if not ids_result:
            return []
        return self._base_get_by_ids(
            operation="get_policy_rules", ids=ids_result, use_params=True, member_cid=member_cid,
        )

    def get_identity_protection_policy_rule_details(
        self,
        ids: list[str] = Field(description="Policy rule IDs to retrieve. Obtain from `falcon_list_identity_protection_policy_rules`."),
        member_cid: str | None = Field(default=None, description="Optional child CID for MSSP scoping."),
    ) -> list[dict[str, Any]] | dict[str, Any]:
        """Get full details for specific Identity Protection policy rules by ID."""
        if not ids:
            return []
        return self._base_get_by_ids(
            operation="get_policy_rules", ids=ids, use_params=True, member_cid=member_cid,
        )

    def create_identity_protection_policy_rule(
        self,
        name: str = Field(description="Rule name."),
        trigger: str = Field(
            description=(
                "Behavioral trigger condition for the rule. "
                "Examples: `access_via_remote_authentication`, `lateral_movement`, `privilege_escalation`."
            ),
        ),
        enabled: bool = Field(default=True, description="Enable the rule on creation."),
        simulation_mode: bool = Field(
            default=False,
            description="If true, logs matches but does not enforce (safe for initial testing).",
        ),
        source_endpoint: dict[str, Any] | None = Field(
            default=None,
            description="Optional source endpoint criteria (AD entity/group filter dict).",
        ),
        source_user: dict[str, Any] | None = Field(
            default=None,
            description="Optional source user criteria (AD entity/group filter dict).",
        ),
        destination: dict[str, Any] | None = Field(
            default=None,
            description="Optional destination criteria (AD entity/group filter dict).",
        ),
        member_cid: str | None = Field(default=None, description="Optional child CID for MSSP scoping."),
    ) -> list[dict[str, Any]] | dict[str, Any]:
        """Create an Identity Protection policy rule.

        IdP policy rules use Active Directory entity criteria, not Falcon host groups.
        Start with `simulation_mode=true` to validate the rule catches expected activity
        ...
        result = self._base_query_api_call(
            operation="post_policy_rules",
            body_params=body,
            error_message="Failed to create identity protection policy rule",
            member_cid=member_cid,
        )
        if self._is_error(result):
            return [result]
    ...
    def delete_identity_protection_policy_rules(
        self,
        ids: list[str] = Field(description="Policy rule IDs to delete."),
        member_cid: str | None = Field(default=None, description="Optional child CID for MSSP scoping."),
    ) -> list[dict[str, Any]] | dict[str, Any]:
        """Delete Identity Protection policy rules by ID."""
        if not ids:
            return [_format_error_response("`ids` is required.", operation="delete_policy_rules")]
        result = self._base_query_api_call(
            operation="delete_policy_rules",
            query_params={"ids": ids},
            error_message="Failed to delete identity protection policy rules",
            member_cid=member_cid,
        )
        if self._is_error(result):
            return [result]
