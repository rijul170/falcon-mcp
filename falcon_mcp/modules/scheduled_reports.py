...
class ScheduledReportsModule(BaseModule):
    ...
    def retry_report_execution(
        self,
        ids: list[str] = Field(
            description="Report execution IDs to retry. Obtain from `falcon_search_report_executions` for failed executions (status='FAILED').",
        ),
    ) -> list[dict[str, Any]] | dict[str, Any]:
        """Retry failed scheduled report executions.

        Re-queues the specified executions. Only executions in a terminal failed state
        (status='FAILED') can be retried. On success, new execution IDs are returned.
        """
        if not ids:
            from falcon_mcp.common.errors import _format_error_response
            return [_format_error_response("`ids` is required.", operation="report_executions_retry")]
        result = self._base_query_api_call(
            operation="report_executions_retry",
            body_params={"ids": ids},
            error_message="Failed to retry report executions",
        )
        if self._is_error(result):
            return [result]
        return result
